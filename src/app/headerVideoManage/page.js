...
//       const res = await axios.get(`${API_URL}/api/header`);
...
//       if (editingId) {
//         await axios.put(
//           `${API_URL}/api/header/${editingId}`,
//           data
//         );

//         setMessage("✅ Service updated successfully");
//       } else {
//         await axios.post(`${API_URL}/api/header`, data);
...
//     try {
//       await axios.delete(`${API_URL}/api/header/${id}`);
...
function Dashboard({ onLogout }) {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get(`${API_URL}/api/services/categories`);
      const data = Array.isArray(res.data) ? res.data : [];
      setCategories(data);
    } catch (error) {
      console.error("Category fetch error:", error);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.headerimage.trim()) {
      setMessage("❌ Header Image is required");
      return;
    }
    ...
    try {
      ...
      if (editingId) {
        await axios.put(`${API_URL}/api/header/${editingId}`, payload);
        setMessage("✅ Item updated successfully");
      } else {
        await axios.post(`${API_URL}/api/header`, payload);
      ...
      <div className="sticky top-0 z-30 bg-white border-b shadow-sm">
        <div className="flex flex-col gap-3 px-4 py-4 mx-auto max-w-7xl sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Manage Header Images & Categories</p>
          </div>
      ...
      <main className="px-4 py-8 mx-auto max-w-7xl">
        ...
        <div className="p-6 mb-8 bg-white rounded-xl shadow">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-xl font-bold text-gray-800">
                {editingId ? "Edit Header Image" : "Add New Header Image"}
              </h2>
              <p className="text-sm text-gray-500">Upload header image and select category</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              {/* CATEGORY INPUT WITH AUTO-COMPLETE DROPDOWN */}
              <div>
                <label className="block mb-2 text-sm font-semibold text-gray-700">
                  Category
                </label>
                <div className="relative">
                  <input
                    ...
                    placeholder="e.g. portrait, landscape, wedding"
                    autoComplete="off"
                    className="w-full px-4 py-3 border rounded-lg outline-none focus:ring-2 focus:ring-blue-400"
                  />
              ...
              <div>
                ...
                >
                  {imageUploading ? "Uploading..." : "Browse & Upload Image"}
                  <input
                    type="file"
                    accept="image/jpeg,image/png,image/webp"
                    className="hidden"
                ...
                {form.headerimage && (
                  <div className="relative mt-3">
                    <img
                      ...
                      alt="Header Preview"
                      className="object-cover w-full h-40 rounded-lg border"
                    />
                    <button
                      type="button"
                      onClick={() => setForm((prev) => ({ ...prev, headerimage: "" }))}
                      className="absolute px-3 py-1 text-xs font-medium text-white bg-red-500 rounded-lg top-2 right-2 hover:bg-red-600"
                    >
                      Remove
                    </button>
        ...
        <div className="p-6 bg-white rounded-xl shadow">
          <div className="flex flex-col gap-4 mb-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h2 className="text-xl font-bold text-gray-800">All Header Items</h2>
              <p className="text-sm text-gray-500">Total: {services.length} items</p>
            </div>
          ...
          {/* LIST CONTAINER */}
          {loading ? (
            <div className="py-12 text-center text-gray-500">Loading headers...</div>
          ) : filteredServices.length === 0 ? (
            <div className="py-12 text-center text-gray-500">No header items found.</div>
          ) : (
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredServices.map((item) => {
                const itemId = item._id || item.id;
                return (
                  <div key={itemId} className="overflow-hidden border rounded-xl bg-gray-50 shadow-sm flex flex-col justify-between">
                    <div>
                      <img
                      ...
                      />
                      <div className="p-4">
                        <span className="inline-block px-3 py-1 text-xs font-semibold text-blue-800 bg-blue-100 rounded-full capitalize">
                          {item.category}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 p-4 border-t bg-white">
                      <button
                        ...
                        className="flex-1 py-2 font-medium text-white bg-blue-500 rounded-lg hover:bg-blue-600 text-sm"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(itemId)}
                        className="flex-1 py-2 font-medium text-white bg-red-500 rounded-lg hover:bg-red-600 text-sm"
                      >
                        Delete
                      </button>
                    </div>
