...
// Social Links API ফেচ (Auto-Retry and Graceful Error Handling)
  useEffect(() => {
    let isMounted = true;

    const fetchSocialsWithRetry = async (retries = 3, delay = 1000) => {
      try {
        ...
        // যদি সার্ভার ৫০০ এরর দেয় এবং রিট্রাই অবশিষ্ট থাকে
        if (retries > 0) {
          setTimeout(() => {
            fetchSocialsWithRetry(retries - 1, delay * 1.5);
          }, delay);
        } else {
          console.error("Failed to fetch socials data after multiple attempts:", err);
          if (isMounted) setSocials([]); // ক্র্যাশ রোধ করতে খালি অ্যারে সেট
        }
      }
    };

    fetchSocialsWithRetry();
  ...
  return (
    <div className="relative w-full min-h-screen lg:min-h-[650px] bg-black text-white py-12 px-4 sm:px-6 md:px-12 flex items-center justify-center overflow-hidden">
      ...
      {/* Main Grid Container */}
  <div className="relative z-10 max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 items-center">
        ...
        <motion.div
          ...
          className="md:col-span-1 lg:col-span-5 xl:col-span-4 bg-zinc-950/80 border border-amber-500/20 backdrop-blur-xl rounded-3xl p-5 flex flex-col items-center text-center shadow-[0_0_40px_rgba(245,158,11,0.06)] hover:border-amber-500/50 hover:shadow-[0_0_50px_rgba(245,158,11,0.15)] transition-all duration-500 group relative"
        >
        ...
        <motion.div
          ...
          className="md:col-span-1 lg:col-span-7 xl:col-span-4 flex flex-col items-center text-center my-4 lg:my-0"
        >
          <div
        ...
        <motion.div
          ...
          className="md:col-span-2 lg:col-span-12 xl:col-span-4 flex flex-col gap-4"
        >
